...
def _attn_fwd_inner(
    ...
    P_block_ptr=None,  # 新增参数
    p_row_offset: tl.constexpr = 0,  # 新增参数
    p_col_offset: tl.constexpr = 0,  # 新增参数
):
    ...
    for start_kv in range(lo, hi, BLOCK_SIZE_KV):
        ...
        # --- 新增：写出 P_block ---
        if P_block_ptr is not None:
            # 计算全局写入位置
            row_idx = p_row_offset + tl.arange(0, BLOCK_SIZE_Q)
            col_idx = p_col_offset + start_kv + tl.arange(0, BLOCK_SIZE_KV)
            # 只写入合法范围
            # valid_row = row_idx < SEQ_LEN
            # valid_col = col_idx < SEQ_LEN
            # # 生成二维 mask
            # mask = valid_row[:, None] & valid_col[None, :]
            # meshgrid 方式生成全局索引
            global_row = row_idx[:, None]    # (BLOCK_SIZE_Q, 1)
            global_col = col_idx[None, :]    # (1, BLOCK_SIZE_KV)
            ptr = P_block_ptr + global_row * SEQ_LEN + global_col
            # tl.store(ptr, P_block, mask=mask)
            tl.store(ptr, P_block)
...
def _attn_fwd(
    ...
    P_out,  # 新增：BATCH_SIZE, NUM_HEADS, SEQ_LEN, SEQ_LEN
    stride_Q_batch,
    stride_Q_head,
    stride_Q_seq,
    stride_Q_dim,
    ...
    # 新增：P_out 指针
    p_offset = (
        index_batch.to(tl.int64) * stride_P_batch
        + index_head.to(tl.int64) * stride_P_head
        + (block_index_q * BLOCK_SIZE_Q) * stride_P_row
    )
    P_block_ptr = P_out + p_offset
...
class TritonAttention(torch.autograd.Function):
    ...
    def forward(ctx, Q, K, V, causal, softmax_scale):
        ...
        # 新增：分配 P_out
        P_out = torch.empty(
            (BATCH_SIZE, NUM_HEADS, SEQ_LEN, KV_LEN), device=Q.device, dtype=torch.float16
        )
        ...
        _attn_fwd[grid](
            ...
            stride_P_batch=P_out.stride(0),  # 新增
            stride_P_head=P_out.stride(1),   # 新增
            stride_P_row=P_out.stride(2),    # 新增
            stride_P_col=P_out.stride(3),    # 新增
        ...
        ctx.causal = causal
        return O, P_out  # 返回 attention scores
