...
from flexmem import Stream_LlavaQwenForCausalLM
from llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from llava.utils import rank0_print
from llava.model.utils import patch_llama
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                or "llava-v1.6-34b" in model_name.lower()
                or "llava-v1.5" in model_name.lower()
                or "internvl_2_8b" in model_name.lower()
            ):
                from llava.model.language_model.llava_llama import LlavaConfig
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                        if "stream" in model_name.lower():
                            model = Stream_LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                        else:
                ...
                try:
                    ...
                    if "stream" in model_name.lower():
                        model = Stream_LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                    else:
                        model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                except:
                    raise ValueError(f"Model {model_name} not supported")
